...
function renderEdit(id){
    ...
    for (let i = 0; i < todoList.length; i++) {
        let task = todoList[i]
        if (id == task.id) {
            html += '<li>' + `<input id="editInput" value="${task.value}" style="margin-top: -9px">` + `<button onClick="submitEdit(${task.id}, document.getElementById('editInput').value)" class="edit">submit</button><button onclick="deleteTask(${task.id})" class="close">delete</button></li>`
        } else {
            html += '<li>' + task.value + `<button onClick="renderEdit(${task.id})" class="edit">edit</button><button onclick="deleteTask(${task.id})" class="close">delete</button></li>`
        }
    }
